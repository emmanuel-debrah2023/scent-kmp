    ...
    route("/api/v1/auth") {
        ...
        post("/login") {
            ...
            val user = transaction {
                UsersTable.selectAll().where { UsersTable.email eq request.email }
                    .map {
                        AuthUserRow(
        ...
        post("/google") {
            ...
            val userId = transaction {
                ...
                existing?.get(UsersTable.id)?.value ?: UsersTable.insertAndGetId {
                    ...
                    it[UsersTable.username] = email.substringBefore("@") + "_" + UUID.randomUUID().toString().take(4)
        ...
        post("/apple") {
            ...
            val jwk = try {
                ...
                return@post call.respond(HttpStatusCode.Unauthorized, ErrorResponse("Failed to fetch Apple public keys"))
            }
            ...
            val userId = transaction {
                ...
                existing?.get(UsersTable.id)?.value ?: UsersTable.insertAndGetId {
                    ...
                    it[UsersTable.username] = "apple_" + appleId.take(10) + "_" + UUID.randomUUID().toString().take(4)
                    it[UsersTable.createdAt] = Clock.System.now().toLocalDateTime(TimeZone.currentSystemDefault())
                }.value
            }
